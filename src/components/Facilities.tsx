...
const iconMap: Record<string, React.ReactNode> = {
  Wifi: <Wifi className="w-6 h-6 text-rose-400" />,
  Wind: <Wind className="w-6 h-6 text-rose-400" />,
  Droplets: <Droplets className="w-6 h-6 text-rose-400" />,
  VolumeX: <VolumeX className="w-6 h-6 text-rose-400" />,
  Zap: <Zap className="w-6 h-6 text-rose-400" />,
  Coffee: <Coffee className="w-6 h-6 text-rose-400" />,
  ShieldCheck: <ShieldCheck className="w-6 h-6 text-rose-400" />,
  Lock: <Lock className="w-6 h-6 text-rose-400" />,
};
...
export const Facilities: React.FC<FacilitiesProps> = ({ onOpenBooking }) => {
  return (
    <section id="facilities" className="py-20 relative bg-rose-950/20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {FACILITIES.map((facility) => (
            ...
            >
              <div>
                <div className="flex items-center justify-between mb-4">
                  <div className="w-12 h-12 rounded-xl bg-rose-500/10 border border-rose-500/30 flex items-center justify-center group-hover:scale-110 transition-transform">
                    {iconMap[facility.iconName] || <Check className="w-6 h-6 text-rose-400" />}
                  </div>
                ...
                <h3 className="text-lg font-bold text-white mb-2 font-['Outfit'] group-hover:text-rose-400 transition-colors">
                  {facility.title}
                </h3>
              ...
              <div className="mt-4 pt-3 border-t border-rose-900/40 flex items-center justify-between text-[11px] text-rose-300 font-medium">
        ...
        {/* Feature Banner Bar */}
        <div className="mt-12 glass-card p-6 sm:p-8 rounded-3xl border border-rose-500/30 bg-gradient-to-r from-rose-950/80 via-slate-900 to-rose-950/80 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <button
            ...
            className="px-6 py-3 rounded-xl bg-gradient-to-r from-rose-600 to-amber-500 hover:from-rose-500 hover:to-amber-400 text-white font-extrabold text-xs tracking-wider uppercase shadow-lg shadow-rose-600/30 transition-all shrink-0"
          >
            Lock Your Seat Now
          </button>
        </div>
